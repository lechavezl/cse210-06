...
from game.scripting.collide_racket2_action import CollideRacket2Action
from game.scripting.control_racket_action import ControlRacketAction
from game.scripting.control_racket2_action import ControlRacket2Action
...
from game.scripting.move_racket2_action import MoveRacket2Action
...
class SceneManager:
    ...
    COLLIDE_RACKET2_ACTION = CollideRacket2Action(PHYSICS_SERVICE, AUDIO_SERVICE)
    CONTROL_RACKET2_ACTION = ControlRacket2Action(KEYBOARD_SERVICE)
    ...
    MOVE_RACKET2_ACTION = MoveRacket2Action()
    ...
    def _prepare_in_play(self, cast, script):
        ...
        script.add_action(INPUT, self.CONTROL_RACKET2_ACTION)
        self._add_update_script(script)
        self._add_output_script(script)
    ...
    def _add_racket2(self, cast):
        cast.clear_actors(SECOND_RACKET_GROUP)
        x = CENTER_X - SECOND_RACKET_WIDTH / 2
        y = 50
        position = Point(x, y)
        size = Point(SECOND_RACKET_WIDTH, SECOND_RACKET_HEIGHT)
        velocity = Point(0, 0)
        body = Body(position, size, velocity)
        animation = Animation(SECOND_RACKET_IMAGES, SECOND_RACKET_RATE)
        racket2 = Racket2(body, animation)
        cast.add_actor(SECOND_RACKET_GROUP, racket2)    
